...
export default class AddMoneyScreen extends React.Component {
  ...
  componentDidMount() {
    let getParamData = this.props.route.params.allData
    let providers = this.props.route.params.providers
  ...
  newData = ({ item, index }) => {
    return (
      <TouchableOpacity style={[styles.boxView, { backgroundColor: item.selected ? colors.GREY.default : '#e6e6e6' }]} onPress={() => { this.quckAdd(index); }}><Text style={styles.quckMoneyText, { color: item.selected ? '#fff' : '#000' }} >{item.amount}{this.state.settings.symbol}</Text></TouchableOpacity>
    )
  }
    ...
    return (
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.mainView}>
        ...
        <View style={styles.bodyContainer}>
          <Text style={styles.walletbalText}>{languageJSON.Balance} - <Text style={styles.ballance}>{this.state.allData ? parseFloat(this.state.allData.walletBalance).toFixed(0) : ''}{this.state.settings.symbol}</Text></Text>
