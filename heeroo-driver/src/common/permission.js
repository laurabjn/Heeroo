...
export async function checkLocationPermission() {
    if (Platform.OS === "ios") {
        ...
        await check(PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION)
            .then(async (result) => {
                console.log('check ACCESS_FINE_LOCATION ')
                if (result == RESULTS.DENIED) {
                    console.log('The permission has not been requested / is denied but requestable');
        ...
        await check(PERMISSIONS.ANDROID.ACCESS_BACKGROUND_LOCATION)
            .then(async (result) => {
                console.log('check ACCESS_BACKGROUND_LOCATION result = ', result)
                if (result == RESULTS.DENIED) {
                    console.log('The permission has not been requested / is denied but requestable');
                    await request(PERMISSIONS.ANDROID.ACCESS_BACKGROUND_LOCATION).then((result) => {
                        if (result == RESULTS.GRANTED) {
                            return true
                        } else {
                            return false
