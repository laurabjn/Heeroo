...
import { google_map_key, exchange_access_key } from "../config/keys";
...
const Dashboard = () => {
    ...
    useEffect(() => {
        if (bookingdata.bookings) {
            ...
            fetch(`http://api.exchangeratesapi.io/v1/latest?access_key=${exchange_access_key}&base=EUR`, {
                type: 'GET',
            })
                .then(data => data.json(),
                )
                .then(res => {
                    if (res.error) {
                        ...
                        for (let i = 0; i < bookingdata.bookings.length; i++) {
                            ...
                            country = bookingdata.bookings[i].pickup.country
                            currency = countryCurrency[country]
                            currencyRate = res.rates[currency]

                            let { trip_cost, discount_amount, tripdate, convenience_fees } = bookingdata.bookings[i];
                            let tDate = new Date(tripdate);

                            trip_cost /= currencyRate
                            discount_amount /= currencyRate
                            convenience_fees /= currencyRate
        ...
        bookingdata.loading || usersdata.loading ? <CircularLoading /> :
            <div>
                ...
                <Grid container direction="row" spacing={2}>
                    <Grid item xs>
                        <DashboardCard title={languageJson.today_text} image={require("../assets/money1.jpg")}>{'€ ' + dailygross}</DashboardCard>
                    </Grid>
                    <Grid item xs>
                        <DashboardCard title={languageJson.this_month_text} image={require("../assets/money2.jpg")}>{'€ ' + monthlygross}</DashboardCard>
                    </Grid>
                    <Grid item xs>
                        <DashboardCard title={languageJson.total} image={require("../assets/money3.jpg")}>{'€ ' + totalgross}</DashboardCard>
                    </Grid>
                </Grid>
                {mylocation ?
